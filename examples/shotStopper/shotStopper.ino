...
bool timerOnly = false;             // Enable timer only functionality
...
BLEByteCharacteristic timerOnlyCharacteristic("0xFF1A",  BLEWrite | BLERead); // not implemented in app
...
void loadOrInitEEPROM() {
  ...
  if (EEPROM.read(SIGNATURE_ADDR) != SIGNATURE_VALUE) {
    ...
    EEPROM.write(TIMER_ONLY_ADDR, timerOnly ? 1 : 0);
    EEPROM.commit();
    ...
    timerOnly = EEPROM.read(TIMER_ONLY_ADDR) != 0;
  }
}
...
void initializeBLE() {
  ...
  shotStopperService.addCharacteristic(timerOnlyCharacteristic);
  ...
  timerOnlyCharacteristic.writeValue(timerOnly ? 1 : 0);
  BLE.advertise();
...
void pollAndReadBLE() {
  ...
  if (timerOnlyCharacteristic.written()) {
    timerOnly = timerOnlyCharacteristic.value();
    EEPROM.write(TIMER_ONLY_ADDR, timerOnly ? 1 : 0);
    updated = true;
  }
  if (updated) {
...
void loop() {
  ...
  && shot.brewing
  && shot.shotTimer > maxShotDurationS ){
    shot.brewing = false;
